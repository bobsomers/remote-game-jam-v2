...
local OlmecBullet = Class(function(self, media, collider, position, direction)
    self.collider = collider
    self.direction = direction

    self.SIZE = Vector(13, 19)
    self.SPEED = Constants.LASER_SPEED

    self.shape = self.collider:addRectangle(0, 0, self.SIZE.x, self.SIZE.y)
    self.shape.kind = "bullet"
    self.collider:addToGroup("foe", self.shape)
    self.shape:moveTo(position.x, position.y)

    self.image = media.OLMEC_BULLET
...
function OlmecBullet:reset()
    self.dead = false
    self.zombie = false
end

function OlmecBullet:kill()
    self.collider:remove(self.shape)
    self.zombie = true
end

function OlmecBullet:update(dt)
